...
pdf("Heatmap_OH_anonymous.pdf", paper="letter", bg="transparent",colormodel="cmyk", pagecentre=TRUE)
heatmap.2(OHmydata, col=mypallete,scale="col",rowsep=1:34, colsep=1:8, sepcol="white",sepwidth=c(.015,.025),trace="none",labRow=OHdata$Number,margins=c(10,10),cexRow=.75,cexCol=1,keysize=2,add.expr=eval(abline(v=5.5, lwd=3, col="yellow"), abline(h=21.5, lwd=3, col="yellow")),colRow=RowColors,lhei=c(3,10))
dev.off()
...
pdf("Heatmap_WVE_anonymous.pdf", paper="letter", bg="transparent",colormodel="cmyk", pagecentre=TRUE)
heatmap.2(WVEmydata, col=mypallete,scale="col",rowsep=1:65, colsep=1:8, sepcol="white",sepwidth=c(.015,.025),trace="none",labRow=WVEdata$Name,margins=c(10,10),cexRow=.75,cexCol=1,keysize=2, add.expr=eval(abline(v=5.5, lwd=3, col="yellow"), abline(h=12.5, lwd=3, col="yellow")),colRow=RowColors,lhei=c(3,10))
dev.off()
...
heatmap.2(Overallmydata, col=mypallete,scale="col",rowsep=1:65, colsep=1:8, sepcol="white",sepwidth=c(.015,.025),trace="none",labRow=merged_data$Name,margins=c(10,10),cexRow=.75,cexCol=1,keysize=2, add.expr=eval(abline(v=5.5, lwd=3, col="yellow"), abline(h=39.5, lwd=3, col="yellow")),colRow=RowColors,lhei=c(3,10))
dev.off()

pdf("Heatmap_Overall_anonymous.pdf", paper="letter", bg="transparent",colormodel="cmyk", pagecentre=TRUE)
heatmap.2(Overallmydata, col=mypallete,scale="col",rowsep=1:65, colsep=1:8, sepcol="white",sepwidth=c(.015,.025),trace="none",labRow=merged_data$Name,margins=c(10,10),cexRow=.75,cexCol=1,keysize=2, add.expr=eval(abline(v=5.5, lwd=3, col="yellow"), abline(h=39.5, lwd=3, col="yellow")),colRow=RowColors,lhei=c(3,10))
dev.off()
...
pdf("RegressionTree_Academics_Separated.pdf", paper="letter", bg="transparent",colormodel="cmyk", pagecentre=TRUE)
rpart.plot(fit1,type=2,extra=2, branch.lty=1, shadow.col="gray", nn=TRUE, under=TRUE,tweak=.75,main="Decision Tree (Academic Testing Separated)")
dev.off()
...
pdf("RegressionTree_Academics_Combined.pdf", paper="letter", bg="transparent",colormodel="cmyk", pagecentre=TRUE)
rpart.plot(fit2,type=2,extra=2, branch.lty=1, shadow.col="gray", nn=TRUE, under=TRUE,tweak=.75,main="Decision Tree (Academic Testing Present)")
dev.off()
...
rpart.plot(fit3,type=2,extra=2, branch.lty=1, shadow.col="gray", nn=TRUE, under=TRUE,tweak=.75,main="Decision Tree (Academic Testing Absent)")
dev.off()
...
print("K means cross validation - 25 v 36")
svm.fit7<-svm(Outcome~Adaptive+FSIQ+WJIII+CBM+SocioEmotional,data=na.omit(merged_data),kernel="polynomial",cross=25,probability=TRUE)
print(svm.fit7)
print(summary(svm.fit7))
print(predict(svm.fit7))
...
print("K means cross validation - 15 v 46")
svm.fit6<-svm(Outcome~Adaptive+FSIQ+WJIII+CBM+SocioEmotional,data=na.omit(merged_data),kernel="polynomial",cross=15,probability=TRUE)
print(svm.fit6)
print(summary(svm.fit6))
print(predict(svm.fit6))
